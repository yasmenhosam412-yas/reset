-- Fix duplicate "liked your post" notifications when toggling like repeatedly.
-- 1) Detect prior membership in OLD.likes using array text elements (case-insensitive),
--    not jsonb @> to_jsonb(...) which can miss how UUIDs are stored in JSON.
-- 2) Skip inbox + push if we already logged a post_like for this recipient + post + actor
--    (covers unlike → like again without spamming the post owner).
...
begin
  ...
  select exists (
    ...
    where lower(trim(t.uid_text)) = lower(trim(liker::text))
